    ...
    <b-container>
      <b-table striped over :items="projects" :fields="fields">
        <template v-slot:cell(actions)="row">
          ...
          <nuxt-link class="btn btn-link" :to="`/projects/${row.item.id}/edit`">Atualizar</nuxt-link>
...
<script>
    export default {
        ...
        created () {
            this.$axios.$get("http://localhost:8080/plataformaEstruturasMetalicas/api/projects/")
        ...
        methods: {
            destroy(id) {
                this.$axios.$delete(`http://localhost:8080/plataformaEstruturasMetalicas/api/projects/${id}`).then(response => {
                    this.$axios.$get("http://localhost:8080/plataformaEstruturasMetalicas/api/projects")
